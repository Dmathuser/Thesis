...
%All three agents will start in the same state. All stochastic transitions will use the same randomly generated probability for all three agents. This allows for a more accurate comparison as each algorithm would have the exact same outcome if they chose the same actions.


%The internal model accuracy is the missing Information equation.
%The learning rate is the time until the model can choose the optimal path to get to a given location.
%The distraction rate is given over all episodes. (Use same time scale as internal model accuracy uses)
%Stop training when internal model is perfectly accurate? (Not possible due to rounding errors)
\begin{figure}
	\begin{center}
...
\begin{figure}[p]
	%Figure~\ref{images/Epsilon_Graphs.pdf}
	%\includegraphics{images/Epsilon_Graphs.pdf}
